...
import { wbadd2703Data } from "@/lib/checkinData";
import { t, type Lang } from "@/lib/wbadd2703Translations";
...
export default function Wbadd2703CheckIn() {
  ...
  const translatedSteps = wbadd2703Data.steps.map((step, i) => ({
    ...step,
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <span className="text-foreground font-semibold md:text-lg">{wbadd2703Data.address}</span>
              </div>
            </div>
            <Link href={wbadd2703Data.mapUrl} target="_blank">
        ...
        <section ref={instructionsRef} className="space-y-8 scroll-mt-24">
          <div className="flex items-end justify-between border-b border-border/40 pb-6">
            ...
            <div className="hidden md:flex items-center gap-2 text-primary font-bold text-xs uppercase tracking-tight">
              {tx.arrivalFrom} {wbadd2703Data.checkInTime}
            </div>
          </div>
            ...
            <InfoCard title={tx.times} icon={Clock}>
              <div className="space-y-3">
                <div className="flex justify-between items-center bg-muted/30 p-2 rounded-lg">
                  ...
                  <span className="text-foreground font-bold">{wbadd2703Data.checkInTime}</span>
                </div>
                <div className="flex justify-between items-center bg-muted/30 p-2 rounded-lg">
                  <span className="text-xs font-bold uppercase text-muted-foreground">Check-out</span>
                  <span className="text-foreground font-bold">{wbadd2703Data.checkOutTime}</span>
                </div>
              </div>
            </InfoCard>
          </div>
        ...
        <section ref={contactRef} className="p-8 rounded-[2rem] bg-foreground text-background scroll-mt-24">
          <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
            <div className="relative h-24 w-24 md:h-32 md:w-32 rounded-3xl overflow-hidden shrink-0 shadow-lg border-2 border-white/10">
              <Image 
                src={wbadd2703Data.host.imageUrl}
                alt={wbadd2703Data.host.name}
                fill
                className="object-cover"
              />
            </div>
            <div className="space-y-4 flex-1">
              <div>
                <h2 className="font-serif text-2xl md:text-3xl font-bold mb-1">{wbadd2703Data.host.name}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-10">
            <Link href={`tel:${wbadd2703Data.host.phone}`}>
            ...
            <Link 
              href={`https://wa.me/${wbadd2703Data.host.phone.replace(/\+/g, "")}?text=${encodeURIComponent(tx.whatsappMessage)}`}
              target="_blank"
            >
