...
export async function syncReservations(startDate, endDate) {
    ...
    if (hospitablePropertyIds.length === 0)
        return 0;
    let count = 0;
    for (const propertyId of hospitablePropertyIds) {
        ...
        for (const res of reservations) {
            ...
            if (res.financials) {
                ...
                const cleaningFee = fin.guest?.fees?.find((f) => f.category === "Guest fees")?.amount || 0;
                const tax = fin.guest?.taxes?.reduce((sum, t) => sum + (t.amount || 0), 0) || 0;
                const totalPaidByGuest = getAmount(fin.guest?.total_price);
                const hostFee = fin.host?.host_fees?.reduce((sum, f) => sum + (f.amount || 0), 0) || 0;
                let payout = getAmount(fin.host?.revenue);
                let calculatedTax = fin.guest?.taxes?.reduce((sum, t) => sum + (t.amount || 0), 0) || 0;
